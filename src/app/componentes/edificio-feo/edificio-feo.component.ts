...
import { IonTabButton, ToastController, IonIcon, IonHeader, IonTabs, IonTab, IonToolbar, IonTitle, IonContent, IonTabBar, IonLabel, IonBadge, IonSegmentButton, IonSegment, IonAvatar, IonItem, IonCard, IonRow, IonText, IonButton, IonButtons, IonFooter, IonImg } from "@ionic/angular/standalone";
...
@Component({
  selector: 'app-edificio-feo',
  templateUrl: './edificio-feo.component.html',
  styleUrls: ['./edificio-feo.component.scss'],
...
export class EdificioFeoComponent  implements OnInit {
  ...
  private toastCtrl: ToastController = inject(ToastController);
   ...
   constructor() {
     ...
     async cargarFotos() {
       const fotos = await this.fotoService.obtenerFotosPorTipo('fea');
   ...
   irGrafico(){
    this.router.navigate(['/grafico-barra'], {
      state: { fotos: this.fotosLindas }
    });
   }
 ...
 async subirFeo() {
   const subidaExitosa = await this.fotoService.tomarYSubirFoto('fea');
   if (subidaExitosa) {
